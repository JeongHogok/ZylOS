...
var ZylServices = (function () {
  ...
  var fs = {
    ...
    /* 서비스 메서드 — Tauri 모드에서는 실제 FS, 아니면 하드코딩 폴백 */
    ...
    getAllData: function () {
      if (IS_TAURI) {
        return tauriInvoke('fs_read_dir', { path: '/' }).then(function (rootEntries) {
          return { tree: { '/': rootEntries || [] }, unixTree: {}, fileContents: {} };
        }).catch(function () {
  ...
  var storage = {
    ...
    _fetchFromHal: function () {
      ...
      return Promise.resolve({ total: 0, used: 0, available: 0, percent: 0 });
    },
      ...
      if (IS_TAURI) {
        return tauriInvoke('list_installed_apps').then(function (list) {
          if (list && list.length > 0) {
            apps.installed = list.map(function (m) {
              return {
                id: m.id,
        ...
        }).catch(function () { return apps.installed; });
      }
      return apps.installed;
    },
    getById: function (id) {
  ...
  var settings = {
    ...
    _loadFromBackend: function () {
      if (!IS_TAURI || settings._loaded) return;
